...
export default function PriceHistoryTable({ changes }: Props) {
  if (changes.length === 0) {
    return (
      <div className="rounded-xl border border-dashed border-gray-300 dark:border-gray-600 py-16 text-center text-gray-400 dark:text-gray-500">
        ...
        <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
          {sorted.map((c, i) => (
            <tr key={i} className="hover:bg-gray-50 dark:hover:bg-gray-800/50">
              <td className="px-4 py-3 text-gray-500 dark:text-gray-400">{formatDate(c.date)}</td>
              <td className="px-4 py-3 font-medium text-gray-900 dark:text-gray-100">{c.provider}</td>
              <td className="px-4 py-3 text-gray-700 dark:text-gray-300">{c.model_or_plan}</td>
              <td className="px-4 py-3">
                <span className={cn(
                  "rounded-full px-2 py-0.5 text-xs font-medium",
                  c.type === "api" ? "bg-blue-100 text-blue-700 dark:bg-blue-900/50 dark:text-blue-300" : "bg-purple-100 text-purple-700 dark:bg-purple-900/50 dark:text-purple-300"
                )}>
                  {c.type === "api" ? "API" : "Subscription"}
                </span>
              </td>
              <td className="px-4 py-3 text-right font-mono text-gray-500 dark:text-gray-400">${c.old_value}</td>
              <td className="px-4 py-3 text-right font-mono font-semibold text-gray-900 dark:text-gray-100">${c.new_value}</td>
